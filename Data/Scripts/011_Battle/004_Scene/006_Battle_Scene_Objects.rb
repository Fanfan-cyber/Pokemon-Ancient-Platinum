...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  NAME_SHADOW_COLOR       = Color.new(72, 72, 72)
...
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  def x=(value)
    super
    onPlayerSide = @battler.index.even?
    opp = onPlayerSide ? 0 : 36
    @hpBar.x     = value + @spriteBaseX + 114 - opp
    @expBar.x    = value + @spriteBaseX + 18
    @hpNumbers.x = value + @spriteBaseX + 92
  end

  def y=(value)
    super
    onPlayerSide = @battler.index.even?
    opp = onPlayerSide ? 0 : 6
    @hpBar.y     = value + 48 - opp
    @expBar.y    = value + 78
    @hpNumbers.y = value + 58
  end
  ...
  def draw_name
    ...
    onPlayerSide = @battler.index.even?
    opp = onPlayerSide ? 0 : 16
    pbDrawTextPositions(self.bitmap, [[@battler.name, @spriteBaseX + 18 - nameOffset - opp, 16, :left,
  ...
  def draw_level
    ...
    onPlayerSide = @battler.index.even?
    opp = onPlayerSide ? 0 : 16
    pbDrawImagePositions(self.bitmap, [[_INTL("Graphics/UI/Battle/overlay_lv"), @spriteBaseX + 140-opp, 22]])
    # Level number
    pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 162-opp, 22)
  end
  ...
  def draw_gender
    ...
    pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + 126 - opp, 16, :left, base_color, shadow_color]])
  end
